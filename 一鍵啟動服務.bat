...
:: 檢查是否有 cloudflared.exe
if not exist "local_db_api\cloudflared.exe" (
    echo [警告] 找不到 local_db_api\cloudflared.exe 執行檔！
    echo 請確保您已將 cloudflared.exe 放進 local_db_api 資料夾中。
    echo 您可以從說明書中的連結下載並置入。
    echo.
    pause
    exit /b
)
...
start "Cloudflare 穿牆隧道" cmd /c "cd local_db_api && .\cloudflared.exe tunnel --url http://localhost:8080"
